...
// Petits éléments partagés par les blocs de /live (PerlimpinpinGo).

// Icônes Heroicons (outline, trait 1,8), comme le header du site.
export const ICONS = {
  ...
  transcript: "M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 0 1 .865-.501 48.172 48.172 0 0 0 3.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z",
...
// Photo du candidat (public/photos, champ Candidat.photoUrl) ; à défaut,
// pastille d'initiales.
// `rounded` : forme de l'avatar (cercle par défaut, carré arrondi pour l'en-tête d'une analyse).
export function Avatar({ nom, photoUrl, size = 40, rounded = "rounded-full" }) {
  const style = { width: size, height: size };
  if (photoUrl) {
    return (
      // eslint-disable-next-line @next/next/no-img-element -- comme le reste du site (photos statiques de public/)
      <img src={photoUrl} alt="" aria-hidden="true" style={style} className={`shrink-0 ${rounded} object-cover`} />
    );
  }
  const initiales = nom
    ? nom
  ...
  return (
    <span
      style={style}
      aria-hidden="true"
      className={`flex shrink-0 items-center justify-center ${rounded} bg-zinc-100 text-xs font-semibold text-zinc-500`}
    >
      {initiales}
    </span>
  );
...
// Badge de score coloré selon le barème à 6 paliers du site (src/lib/score.js,
// même source que les fiches publiques).
export function ScoreBadge({ score }) {
  const badge = getScoreBadge(score);
  return (
    <span
      title={badge.label}
      className={`inline-flex shrink-0 items-baseline gap-1 rounded-full px-2.5 py-1 text-sm font-bold ${badge.badgeClass}`}
    >
      {score}
...
// Élément visible mais inactif, avec infobulle. Le title est posé sur un
// conteneur : un <button disabled> n'émet pas d'événements souris, donc son
// propre title ne s'afficherait pas partout.
export function ComingSoon({ children, className = "" }) {
  return (
    <span title="Bientôt disponible" className={`inline-flex ${className}`}>
      {children}
    </span>
  );
}

// Couleur de chaque verdict d'affirmation (ensemble fermé défini dans
// src/lib/live-analyse.js : VERDICTS).
const VERDICT_STYLES = {
  "Cohérent": { dot: "bg-emerald-600", text: "text-emerald-700" },
  "Plutôt cohérent": { dot: "bg-green-500", text: "text-green-600" },
  "Incertain": { dot: "bg-amber-400", text: "text-amber-600" },
  "Non étayé": { dot: "bg-red-500", text: "text-red-600" },
  "Contredit": { dot: "bg-red-800", text: "text-red-800" },
};
const VERDICT_FALLBACK = { dot: "bg-zinc-300", text: "text-zinc-500" };

export function VerdictDot({ verdict }) {
  const style = VERDICT_STYLES[verdict] ?? VERDICT_FALLBACK;
  return <span aria-hidden="true" className={`mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full ${style.dot}`} />;
}

export function VerdictLabel({ verdict, className = "" }) {
  const style = VERDICT_STYLES[verdict] ?? VERDICT_FALLBACK;
  return <span className={`shrink-0 text-sm font-medium ${style.text} ${className}`}>{verdict}</span>;
}

// Remplissage de la barre de score, selon la couleur du palier (getScoreBadge().color,
// src/lib/score.js — source unique des 6 paliers).
